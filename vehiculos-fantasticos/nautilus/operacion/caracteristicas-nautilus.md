...
```mermaid
flowchart TD
    Nautilus[🐙 Nautilus] --> Estructura[Estructura]
    Nautilus --> Flotacion[Flotación]
    Nautilus --> Energia[Energía]
    Nautilus --> Vida[Soporte vital]
    Estructura --> Casco[Casco resistente]
    Estructura --> Forma[Forma hidrodinámica]
    Flotacion --> Lastre[Tanques de lastre]
    Flotacion --> Timones[Timones de profundidad]
    Energia --> Electrica[Fuente eléctrica]
    Energia --> Helice[Propulsión por hélice]
    Vida --> Aire[Renovación del aire]
    Vida --> Reservas[Agua y provisiones]
```
