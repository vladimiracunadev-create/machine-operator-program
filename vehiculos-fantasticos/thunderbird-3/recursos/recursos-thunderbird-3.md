...
```mermaid
flowchart LR
    Vertical[Subir recto] --> Altura[Gana altura]
    Altura --> Cae[Sin velocidad lateral vuelve a caer]
    Horizontal[Inclinar y acelerar] --> Lateral[Gana velocidad lateral]
    Lateral --> Orbita[Cae sin parar y no llega al suelo]
    Orbita --> Idea[Orbitar no es solo subir]
```
