...
codigo: DRONES-03
curso: drones
titulo: "Modelos y variantes del dron"
modalidad: "comparativa guiada"
duracion_minutos: 60
nivel: introductorio
prerrequisito: DRONES-02
competencia: "seleccion_de_configuracion"
resultados_aprendizaje:
  - "Explicar manejo, arquitectura de mandos y variables de simulación con vocabulario propio de Drones."
...
El [Clase 2](../operacion/caracteristicas-dron.md) ya dijo qué tipos de dron
...
El [Clase 5](../mandos/manual-mandos-dron.md) describe un puesto de mando de dos
sticks donde el throttle sube o baja el empuje total y, en el modo común,
controla la altura. El [Clase 9](../simulacion/diseno-simulador-dron.md) expone
...
[Clase 5](../mandos/manual-mandos-dron.md) ya lo advierte: el modo activo
...
| --- | --- | --- |
| Multirotor, cuadricóptero o hexacóptero | Ninguno: el mapa de controles del Clase 5 aplica tal cual. | Cambian los márgenes, no los controles. |
...
| Multirotor en modo GPS / posición | Ninguno nuevo: es el caso base del Clase 5. | El stick pide una posición y el aparato la sostiene al soltarlo. |
...
- **El UGV y el ROV** frente a todo lo anterior: no vuelan. El
  [Clase 2](../operacion/caracteristicas-dron.md) ya los deja fuera del foco, y
  aquí se confirma por qué: no comparten ni una variable del Clase 9.
...
el [Clase 6](../operacion/principios-dron.md), recién en el nivel 3.
...
[Clase 8](../reglamentos/reglamentos-dron.md) y no se usan aquí para definir
...
¿Cómo ayuda **Por qué el modelo decide el simulador, Qué cambia en el manejo, Qué cambia en el mando y Qué cambia en el simulador** a **comparar multirrotor frente a ala fija frente al mismo encargo**?

### Explicación razonada

Las variantes «multirrotor frente a ala fija» resuelven prioridades distintas. Una comparación profesional sigue la cadena batería → controladores → motores y hélices → actitud y trayectoria: cada cambio de arquitectura modifica mandos, respuesta, mantenimiento y variables que una simulación debe representar. Elegir un modelo significa justificar qué compromiso sirve mejor al caso, no declarar un favorito.
...
1. **Mantener el encargo constante:** ambas variantes deben evaluarse ante **inspección próxima a una estructura con viento y señal GNSS degradada**.
2. **Trazar consecuencias:** para cada variante sigue el efecto desde **batería** hasta **actitud y trayectoria**.
...
1. ¿Qué cambia en la cadena **batería → controladores → motores y hélices → actitud y trayectoria** entre las dos variantes?
2. ¿Qué indicación o mando adicional necesitaría una de ellas?
3. ¿Cuál elegirías para «inspección próxima a una estructura con viento y señal GNSS degradada» y qué desventaja aceptarías?
...
- **Actividad:** Compara variantes de Drones mediante los ejes «manejo, arquitectura de mandos y variables de simulación» y elige una para un caso de uso razonado.
- **Evidencia:** Matriz comparativa y decisión justificada.
